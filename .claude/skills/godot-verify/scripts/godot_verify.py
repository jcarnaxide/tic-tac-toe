...
def gdscript_files(root: Path, targets: list[str]) -> list[Path]:
    if targets:
        ...
        return out
    # `.godot/` is the engine's own import cache, addons/ is third-party code
    # the project did not write, and `.claude/` is agent tooling that happens to
    # sit inside the project — none belongs in a project's own error report.
    skip = {".godot", "addons", ".claude"}
    return sorted(
